...
public class UseCarr {
	...
	public static void main(String[] args) {
		...
		cars.put(car4.getBrand(), car4);
		cars.put(car5.getBrand(), car5);
		cars.put(car6.getBrand(), car6);
		cars.put(car7.getBrand(), car7);
		cars.put(car8.getBrand(), car8);
		cars.put(car9.getBrand(), car9);
		cars.put(car10.getBrand(), car10);
		
		cars.values().stream().filter(x-> x.getRating()>4 && x.getRating()<=8).forEach(x-> System.out.println(x));
		
		List <Car> whiteColor = cars.values().stream().filter(x -> x.getColor().equalsIgnoreCase("white") && x.getPrice()>800000).collect(Collectors.toList());
		whiteColor.forEach(x -> System.out.println(x));
		
		List <String> tnCars = cars.values().stream().filter(x-> x.getNoPlate().startsWith("TN")).map(x->x.getBrand()).collect(Collectors.toList());
		tnCars.forEach(x -> System.out.println(x));
		
		Map <String,Car> m = cars.values().stream().filter(x-> x.getCc()<3000 && x.getCc()>2000).collect(Collectors.toMap(x-> x.getNoPlate(), y->y));
